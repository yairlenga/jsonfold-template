...
for id in "${id_list[@]}" ; do
	...
	if [ "$skip_msg" ] ; then
		...
		opt=
		level=${data["$id.LEVEL"]-default}
		case "$level" in
			none) opt="--no-plugins" ;;
			basic | nav | logic | py | default ) ;;
		...
		case "$out_fmt" in
			default) ;;
			stream) jq_filter='.[]' ;;
			*) error=${error:+, }"Bad OUT_FMT=$out_fmt" ;;
		esac
		echo  "Debug: " ../../python/run.sh -v $opt $jf_file $input_file >> $err_file
		if ! ../../python/run.sh $opt $jf_file $input_file 2>> $err_file | jq "$jq_filter" > "$jf_out" 2>> "$err_file" ; then
			error+=${error:+, }"JF fail"
		else
			diff=$(diff $jf_out $gold_file || true)
			if [ "$diff" ] ; then
				(echo "=== jftl-diff" ; echo "$diff") >> $err_file
				error+=${error:+, }"JFTL-DIFF"
			else
				echo "=== jftl-match: OK" >> $err_file
			fi
		fi
		[ "$TRACE" ] && set +x
	fi
