...
4. Type predicates `isarray`, `isobject`, `isscalar`, `isindex`, `isstring`, `isbool`, `isnumber`, `isint`, is null
...
10. Reserve grammar for single argument function calls `foo:bar:baz` and `baz | bar | foo`
11. Fix handling of statements that evaluate to "null". They should return a reference to literal None. None will indicate no body.
12. Basic format for stringification of numbers - decimals, grouping, percent, 
13. Add condition specific prefixes: not: empty: null: array: object:, disallow plain values in condition.
14. Remove "shape" checks - after bundling it to condition.
15. 
